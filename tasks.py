...
def release(c, ver="v3", force=False):
    ...
    if not is_repo_pristine(".") and not force:
    ...
    print(f"..copying files to {destination}/{year}/data-exchange-protocol-{date}")
    ...
    print(f"..copying files to {destination}/data-exchange-protocol/{version}")
    c.run(f"rm -rf {destination}/data-exchange-protocol-{version}")
